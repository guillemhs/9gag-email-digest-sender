...
class Parser9GAG(object):
	...
	def FindLinksNoImage(self): #required for getting page source of base page that is then passed to FindLinks2
	        base_link="http://9gag.com/"
	        base_contents=urllib2.urlopen(base_link).read()
	        s=self.FindLinks2NoImage(base_contents)
		return s

	def FindLinks2(self, base_contents):  #find links like http://9gag.com/gag/6482858 that can be passed to the Extract Image Function
	 	global number
	 	initial=0
	 	breakval=0
	...
	def FindLinks2NoImage(self, base_contents):  #find links like http://9gag.com/gag/6482858
 		global number
	 	initial=0
 		breakval=0
		s=""
 		for i in range (0,3): #keep on finding links until the next link is not equal to first one
  			start_link=base_contents.find('data-entry-url="',initial)+16
	  		end_link=base_contents.find('"',start_link)
	  		link=base_contents[start_link:end_link]
	  		initial=end_link
	...
	def FindTitle(self, temp): #Find title of image to be downloaded
	...
	def Rename(self, actual_link,toName): #Give the downloaded file a more meaningful name
	...
	def ExtractTitle(self, inputUrl): #Will extract image from a page like http://9gag.com/gag/6482858
        	Page_contents=urllib2.urlopen(inputUrl).read()
	        title_of_image=self.FindTitle(Page_contents)
        	return title_of_image


	def ExtractImage(self, inputUrl): #Will extract image from a page like http://9gag.com/gag/6482858
		Page_contents=urllib2.urlopen(inputUrl).read()
	...
	def FindLinks3(self, base_link): #find page source of next hot page and return it 
 		global nextLink
	 	base_contents=urllib2.urlopen(base_link).read()
