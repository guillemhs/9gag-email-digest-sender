...
def FindLinksNoImage(): #required for getting page source of base page that is then passed to FindLinks2
        base_link="http://9gag.com/"
        base_contents=urllib2.urlopen(base_link).read()
        s=FindLinks2NoImage(base_contents)
	return s
...
def FindLinks2NoImage(base_contents):  #find links like http://9gag.com/gag/6482858
 	global number
 	initial=0
 	breakval=0
	s=""
 	for i in range (0,3): #keep on finding links until the next link is not equal to first one
  		...
  		if (initial==breakval and i!=0):
   			...
   			break
  		temp=str(number)+". "+link
		print "<a href=\""+ link +"\">"+ ExtractTitle(link) +"</a><p>"
		s+="<a href=\""+ link +"\">"+ ExtractTitle(link) +"</a><p>"
  		number=number+1
	return str(s)
...
def ExtractTitle(inputUrl): #Will extract image from a page like http://9gag.com/gag/6482858
